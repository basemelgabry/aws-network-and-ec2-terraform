...
resource "aws_route_table" "tf-demortb" {
...
resource "aws_route" "way-to-igw" {
  ...
  route_table_id         = "${aws_route_table.tf-demortb.id}"
  destination_cidr_block = var.aws_route_destination_cidr_block                                  # variable
  gateway_id             = "${aws_internet_gateway.tf-demoigw.id}"
